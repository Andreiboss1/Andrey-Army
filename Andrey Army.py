...
**a!search** : `Search something`
**a!avatar** : `Get a member's avatar`""")
    await ctx.author.send("""
**a!playerinfo @<member>** : `Get a member's informations`
**a!serverinfo** : `Get a server's informations`
**a!botinfo** : `Check the BOT info`
**a!lenny** : `Returns the lenny face`
**a!respect** : `Press F`
**a!sal** : `Returns a hi`""")
    await ctx.author.send("""
**a!kick** : `Kick a member (Avaible only for members with the Kick permission)`
**a!ban** : `Ban a member (Avaible only for members with the Ban permission)`""")
    await ctx.author.send("""
**a!play** : `Play a song`
**a!stop** : `Stops the track`
**a!queue** : `See the following tracks`
**a!skip** : `Plays the next song`
**a!pause** : `Pause the track`
**a!resume** : `Unpause the track`
**a!join** : `Join a voice channel`""")
...
async def on_message(message : discord.Message):
    if bot.user.mentioned_in(message):
        await message.channel.send(':sleeping: | You woke me up :( . My prefix is `a!` , for a list of commands type `a!help`', delete_after=60)

@bot.listen()
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
            return await ctx.send(f':no_entry: | Hey, You are being ratelimited! Try again in** {int(error.retry_after)} **seconds!', delete_after=10)
...
async def invite(ctx):
    ...
    await ctx.send("Invite the BOT here: https://discordapp.com/api/oauth2/authorize?client_id=461246693363417098&permissions=8&scope=bot")
...
async def botinfo(ctx):
    ...
    em.add_field(name="Creator", value='<@434744215608492032>', inline=True)
